...
Argument is the repo name (one of jax, xla, orbax, tunix, tokamax).

**XLA is special — see step 0 first.**

0. **XLA scope check.** XLA is mostly C++, several backends (CPU/GPU/TPU),
   and ~10× the surface area of the Python repos. A single-session full
   backfill is infeasible — it produces mediocre file-list-y bedrock.
   Use the **subsystem-first** approach instead (per
   `claude-designs/cartograph/xla-bedrock-backfill/`):
   ...
   - The XLA ramp-up skeleton at `learn/ramp-up/xla.md` (if present)
   ...
   - After all chosen subsystems are covered, bump
     ...
     files. Write an episode tagged `xla, backfill, <subsystem>`.

   Then stop. Don't try to "finish" XLA in one session.

For the Python repos (jax / orbax / tunix / tokamax), continue with the
linear flow below.
